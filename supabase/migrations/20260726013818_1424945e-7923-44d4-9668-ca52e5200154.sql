...
CREATE POLICY "Staff can update own profile" ON public.profiles FOR UPDATE TO authenticated
...
-- new user: profile + first-user-is-admin
...
BEGIN
  ...
  VALUES (NEW.id, CASE WHEN existing = 0 THEN 'admin'::public.app_role ELSE 'receptionist'::public.app_role END)
...
CREATE POLICY "Staff manage visitors" ON public.visitors FOR ALL TO authenticated USING (true) WITH CHECK (true);
...
CREATE POLICY "Admins manage config" ON public.facility_config FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Admins delete audit" ON public.audit_logs FOR DELETE TO authenticated USING (public.has_role(auth.uid(), 'admin'));
